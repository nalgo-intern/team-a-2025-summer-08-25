...
def analyze_image():
    ...
    # --- OCR処理 ---
    cuted_image = ci.select_and_crop(input_image)
    extracted_text = ocr.ocr_from_pil_png(cuted_image, run_ocr=run_ocr)

    # ダミーのテキストを返す
    # extracted_text = "画像から抽出したテキストです。今日は良い天気ですね。"

    model_result = es.analyze_text(extracted_text)
    model_result['text'] = wn.span_text(extracted_text)
